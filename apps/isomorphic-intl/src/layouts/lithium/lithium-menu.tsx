...
function EnhancedMenuItems({
  ...
  return (
    <div
      className={cn("w-[calc(100%_-_200px)] rounded-lg bg-white pl-3 dark:bg-gray-100", className)}
    >
  ...
  (
    ...
    useEffect(() => {
      if (items.some((item) => item.subMenuItems?.some((subItem) => subItem.href === pathname))) {
        setState(
          items.findIndex((item) => item.subMenuItems?.some((subItem) => subItem.href === pathname))
        );
      }
    }, [items, pathname]);
    return (
          ...
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs font-normal leading-5 text-gray-500">{item?.description}</p>
                <span
                  className={cn(
