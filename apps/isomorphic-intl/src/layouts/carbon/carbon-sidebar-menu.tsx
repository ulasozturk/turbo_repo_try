      ...
      <ul className="pb-12">
        ...
        >
          {items.map((item, index) => {
            ...
            const pathnameExistInDropdowns: boolean = item?.menuItems?.some((dropdownItem) => {
              if (dropdownItem.href === "/") {
                return pathname === `/${locale}`;
              } else {
                return (
                  `/${locale}${dropdownItem.href}` === pathname ||
                  dropdownItem.subMenuItems?.some(
                    (subMenuItem) => `/${locale}${subMenuItem.href}` === pathname
                  )
                );
              }
            });
                                ...
                                >
                                  ...
                                  <Menu.List className="!border-transparent dark:border-gray-300 dark:bg-gray-100">
                                    {dropdownItem?.subMenuItems?.map((subMenuItem, index) => {
                                      const subMenuItemUrl = `/${locale}${subMenuItem?.href}`;
                                      const isChildActive = pathname === subMenuItemUrl;

                                      return (
                                        <Menu.Item
                                          key={"sub-menu" + subMenuItem?.name + index}
                                          className="px-0 py-0"
                                        >
                                          <Link
